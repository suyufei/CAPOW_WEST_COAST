...
def setup(year,hist,hist_year):
    ...
    df_gen.columns = CA_list
    
    #read transmission path parameters into DataFrame
    df_paths = pd.read_csv('WC_data_file/paths.csv',header=0)
    
    #calendar
    df_calendar = pd.read_excel('WC_data_file/calendar.xlsx',header=0)
    
    #list zones
    zones = ['PGE_valley', 'PGE_bay', 'SCE', 'SDGE','PNW']                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                
    ...
    CA_reserves = np.array(pd.read_csv('CA_data_file/reserves.csv',header=0)['reserves'])
    PNW_reserves = np.array(pd.read_csv('PNW_data_file/reserves.csv',header=0)['reserves'])
    total_reserves = np.sum((CA_reserves,PNW_reserves),axis=0)
    
    
    ##daily hydropower availability 
    df_CA_hydro = pd.read_csv('Hydro_setup/CA_dispatchable_hydro.csv',header=0)
    df_PNW_hydro = pd.read_csv('Hydro_setup/PNW_dispatchable_hydro.csv',header=0)
    ...
    df_CA_wind = df_wind.loc[:,'CAISO']
    df_PNW_wind = df_wind.loc[:,'PNW']
    df_CA_wind = df_CA_wind.loc[year*8760:year*8760+8759]
    df_CA_wind = df_CA_wind.reset_index()
    df_PNW_wind = df_PNW_wind.loc[year*8760:year*8760+8759]
    df_PNW_wind = df_PNW_wind.reset_index()
    ...
    df_CA_imports = pd.read_csv('Path_setup/CA_dispatchable_imports.csv',header=0)
    df_PNW_imports = pd.read_csv('Path_setup/PNW_dispatchable_imports.csv',header=0)
     
    ##hourly time series of exports by zone
    df_CA_exports = pd.read_csv('Path_setup/CA_exports.csv',header=0)
    df_PNW_exports = pd.read_csv('Path_setup/PNW_exports.csv',header=0)
         
    #must run resources (LFG,ag_waste,nuclear)
    df_CA_must = pd.read_excel('CA_data_file/must_run.xlsx',header=0)
    df_PNW_must = pd.read_csv('PNW_data_file/must_run.csv',header=0)
    ...
    if hist > 0:
        ...
        for i in range(0,len(df_calendar)):
            ...
            must_run_SCE = np.append(must_run_SCE,sc+df_CA_must.loc[0,'SCE'])
            must_run_SDGE = np.append(must_run_SDGE,sdc+df_CA_must.loc[0,'SDGE'])
            must_run_PGE_valley = np.append(must_run_PGE_valley,pc+df_CA_must.loc[0,'PGE_valley'])
             
        must_run_PGE_bay = np.ones((len(df_load),1))*df_CA_must.loc[0,'PGE_bay'] 
        must_run_PNW = np.ones((len(df_load),1))*(df_PNW_must.loc[0,'PNW'])  
         
        must_run = np.column_stack((must_run_PGE_valley,must_run_PGE_bay,must_run_SCE,must_run_SDGE,must_run_PNW))
        df_total_must_run =pd.DataFrame(must_run,columns=('PGE_valley','PGE_bay','SCE','SDGE','PNW'))
        df_total_must_run.to_csv('WC_data_file/must_run_hourly.csv')
        
    else:
        
        must_run_PGE_bay = np.ones((len(df_load),1))*df_CA_must.loc[0,'PGE_bay']
        must_run_PGE_valley = np.ones((len(df_load),1))*df_CA_must.loc[0,'PGE_valley']
        must_run_SCE = np.ones((len(df_load),1))*df_CA_must.loc[0,'SCE']
        must_run_SDGE = np.ones((len(df_load),1))*df_CA_must.loc[0,'SDGE']
        must_run_PNW = np.ones((len(df_load),1))*(df_PNW_must.loc[0,'PNW'])  
        must_run = np.column_stack((must_run_PGE_valley,must_run_PGE_bay,must_run_SCE,must_run_SDGE,must_run_PNW))
        df_total_must_run =pd.DataFrame(must_run,columns=('PGE_valley','PGE_bay','SCE','SDGE','PNW'))
        df_total_must_run.to_csv('WC_data_file/must_run_hourly.csv')
    ...
    #write data.dat file
    filename = 'WC_data_file/data.dat'
    with open(filename, 'w') as f:
        ...
        # WECC imports
        f.write('set WECCImportsPNW :=\n')
        # pull relevant generators
        for gen in range(0,len(df_gen)):
            if df_gen.loc[gen,'typ'] == 'imports' and df_gen.loc[gen,'zone'] == 'WECC_PNW':
                unit_name = df_gen.loc[gen,'name']
                unit_name = unit_name.replace(' ','_')
                f.write(unit_name + ' ')
        f.write(';\n\n')
    ...
    # create parameter matrix for generators
        ...
        #system wide (daily)
        f.write('param:' + '\t' + 'SimPath46_SCE_imports' + '\t' + 'SimPath61_imports' + '\t' + 'SimPath42_imports' + '\t' + 'SimPath24_imports' + '\t' + 'SimPath45_imports' + '\t' + 'SimPath3_imports' + '\t' + 'SimPath8_imports' + '\t' + 'SimPath14_imports' + '\t' + 'SimPGE_valley_hydro' + '\t' + 'SimSCE_hydro' + '\t' + 'SimPNW_hydro:=' + '\n')
        for d in range(0,len(df_CA_imports)):
                f.write(str(d+1) + '\t' + str(df_CA_imports.loc[d,'Path46_SCE']) + '\t' + str(df_CA_imports.loc[d,'Path61']) + '\t' + str(df_CA_imports.loc[d,'Path42']) + '\t' + str(df_CA_imports.loc[d,'Path24']) + '\t' + str(df_CA_imports.loc[d,'Path45']) + '\t' + str(df_PNW_imports.loc[d,'Path3']) + '\t' + str(df_PNW_imports.loc[d,'Path8']) + '\t' + str(df_PNW_imports.loc[d,'Path14']) + '\t' + str(df_CA_hydro.loc[d,'PGE_valley']) + '\t' + str(df_CA_hydro.loc[d,'SCE']) + '\t' + str(df_PNW_hydro.loc[d,'PNW']) + '\n')
        f.write(';\n\n')
        
        
        #system wide (hourly)
        f.write('param:' + '\t' + 'SimPath42_exports' + '\t' + 'SimPath24_exports' + '\t' + 'SimPath45_exports' + '\t' + 'SimPath65_exports' + '\t' + 'SimPath3_exports' + '\t' + 'SimPath8_exports' + '\t' + 'SimPath14_exports' + '\t' + 'SimReserves' + '\t' + 'SimSCE_hydro_minflow' + '\t' + 'SimPGE_valley_hydro_minflow' + '\t' + 'SimPNW_hydro_minflow' + '\t' + 'SimPath3_imports_minflow' + '\t' + 'SimPath8_imports_minflow' + '\t' + 'SimPath14_imports_minflow' + '\t' + 'SimPath61_imports_minflow' + '\t' + 'SimPath46_SCE_imports_minflow' + '\t' + 'SimPath42_imports_minflow:=' + '\n')
        for h in range(0,len(df_load)):
                f.write(str(h+1)  + '\t' + str(df_CA_exports.loc[h,'Path42']) + '\t' + str(df_CA_exports.loc[h,'Path24']) + '\t' + str(df_CA_exports.loc[h,'Path45'])+ '\t' + str(df_PNW_exports.loc[h,'Path65']) + '\t' + str(df_PNW_exports.loc[h,'Path3']) + '\t' + str(df_PNW_exports.loc[h,'Path8']) + '\t' + str(df_PNW_exports.loc[h,'Path14']) + '\t' + str(total_reserves[h])  + '\t' + str(df_CA_hydro_mins.loc[h,'SCE']) + '\t' + str(df_CA_hydro_mins.loc[h,'PGE_valley']) + '\t' + str(df_PNW_hydro_mins.loc[h,'PNW']) + '\t' + str(df_PNW_import_mins.loc[h,'Path3']) + '\t' + str(df_PNW_import_mins.loc[h,'Path8']) + '\t' + str(df_PNW_import_mins.loc[h,'Path14']) + '\t' + str(df_CA_import_mins.loc[h,'Path61']) + '\t' + str(df_CA_import_mins.loc[h,'Path46_SCE']) + '\t' + str(df_CA_import_mins.loc[h,'Path42']) + '\n')
        f.write(';\n\n')
