...
model.Coal = Set()
model.Gas = model.Zone1Gas | model.Zone2Gas | model.Zone3Gas | model.Zone4Gas | model.Zone5Gas
model.Oil = Set()
model.PSH = Set()
model.Slack = Set()
model.Hydro = Set()
...
model.WECCImportsPNW = Set()
model.Ramping = model.Hydro | model.WECCImportsPGEV | model.WECCImportsSCE | model.WECCImportsSDGE | model.WECCImportsPNW
   
model.Generators = model.Zone1Generators | model.Zone2Generators | model.Zone3Generators | model.Zone4Generators | model.Zone5Generators | model.WECCImportsPGEV | model.WECCImportsSCE | model.WECCImportsSDGE | model.WECCImportsPNW
...
def SysCost(model):
    ...
    pnw_imports1 = sum(model.mwh_1[j,i]*5 for i in model.hh_periods for j in model.WECCImportsPNW)
    pnw_imports2 = sum(model.mwh_2[j,i]*5 for i in model.hh_periods for j in model.WECCImportsPNW)
    pnw_imports3 = sum(model.mwh_3[j,i]*5 for i in model.hh_periods for j in model.WECCImportsPNW)
    ...
    return fixed + nuc1 + nuc2 + nuc3 + coal1 + coal2 + coal3 + gas1_1 + gas1_2 + gas1_3 + gas1_4 + gas2_1 + gas2_2 + gas2_3 + gas2_4 + gas3_1 + gas3_2 + gas3_3 + gas3_4 + gas1_5 + gas2_5 + gas3_5 + sce_imports1 + sce_imports2 + sce_imports3 + sdge_imports1 + sdge_imports2 + sdge_imports3 + pgev_imports1 + pgev_imports2 + pgev_imports3 + pnw_imports1 + pnw_imports2 + pnw_imports3 + oil1 + oil2 + oil3 + psh1 + psh2 + psh3 + slack1 + slack2 + slack3 + starts + exchange
...
def WECC5(model,i):
    ...
    renew = model.solar['PNW',i] + model.wind['PNW',i] 
    must_run = model.HorizonMustRun['PNW',i]
    return s1 + s2 + s3 + renew + must_run >=  0.25*model.HorizonDemand['PNW',i]
...
def Zone1_Balance(model,i):
    ...
    imports = sum(model.flow[s,'PGE_valley',i] for s in model.sources) + model.mwh_1['P24I',i] + model.mwh_2['P24I',i] + model.mwh_3['P24I',i]
    exports = sum(model.flow['PGE_valley',k,i] for k in model.sinks) + model.HorizonPath24_exports[i]      
...
model.ImportsConstraint13= Constraint(model.h1_periods,rule=ImportsC13)

def ImportsC14(model,i):
...
model.ImportsConstraint14= Constraint(model.h2_periods,rule=ImportsC14)

def ImportsC15(model,i):
...
model.ImportsConstraint15= Constraint(model.h1_periods,rule=ImportsC15)

def ImportsC16(model,i):
...
model.ImportsConstraint16= Constraint(model.h2_periods,rule=ImportsC16)

def ImportsC17(model,i):
...
model.ImportsConstraint17= Constraint(model.h1_periods,rule=ImportsC17)

def ImportsC18(model,i):
...
model.ImportsConstraint18= Constraint(model.h2_periods,rule=ImportsC18)
