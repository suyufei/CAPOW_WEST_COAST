...
# Simulation data
#sim_weather=pd.read_csv('Synthetic_weather/WIND_TEMP.csv',header=0)
sim_weather=pd.read_csv('Synthetic_weather/synthetic_weather_data.csv',header=0)

title_fill=list(sim_weather)[1:]
...
#de2=np.vstack(DE)
his=np.zeros(num_gages)
syn=np.zeros(num_gages)
regre=np.zeros(num_gages)
raw_DE=np.zeros(num_gages)
for i in range(0,num_gages):
    his[i]=np.mean(totals_hist[i,:])
    syn[i]=np.mean(sim_totals[:,i])
    regre[i]=np.mean(P[:,i])
#    raw_DE[i]=np.mean(de2[i,:])
